...
const PGMMapLoader: React.FC<PGMMapLoaderProps> = (props) => {
  ...
  const { canUndo, canRedo, saveToUndoStack, handleUndo, handleRedo } = useUndoStack()
  ...
  const onRedo = useCallback(() => {
    if (mapData) {
      const nextState = handleRedo(handleRotationChange)
      if (nextState) {
        setMapData(nextState)
      }
    }
  }, [mapData, handleRedo, handleRotationChange])
